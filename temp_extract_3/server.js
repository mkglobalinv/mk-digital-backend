...
app.post("/buy-electricity", auth, async (req, res) => {
  try {
    const { amount, phone, discoId, meterType, meterNumber } = req.body;
    if (!amount || amount <= 0) return res.status(400).json({ message: "Invalid amount" });
    if (!phone || !discoId || !meterType || !meterNumber) return res.status(400).json({ message: "Missing details" });
    ...
    if (!deductedUser) return res.status(400).json({ message: "Insufficient balance" });

    let vtuResponse;
    try {
        vtuResponse = await buyElectricity(discoId, meterType, meterNumber, amount, phone);
    } catch(err) {
        await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
        return res.status(500).json({ message: "Service unavailable. Refunded." });
    }

    if (vtuResponse.status === "success") {
       await Transaction.create({
         userId: user._id, type: "debit", status: "success",
         amount, phone, network: discoId,
         reference: vtuResponse.reference,
         description: `Electricity (${discoId}) - ${meterNumber}`,
         token: vtuResponse.token
       });
       res.json({ message: "Purchase successful", balance: deductedUser.balance, token: vtuResponse.token });
    } else {
       await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
       res.status(400).json({ message: vtuResponse.message || "Failed. Refunded.", balance: deductedUser.balance + amount });
    }
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
...
app.post("/buy-epin", auth, async (req, res) => {
  try {
    const { amount, network, quantity } = req.body; // Total amount = Value * Quantity
    const value = amount / quantity;
    if (!amount || amount <= 0 || !quantity) return res.status(400).json({ message: "Invalid details" });

    const user = await User.findById(req.user.id);
    if (user.balance < amount) return res.status(400).json({ message: "Insufficient balance" });

    const deductedUser = await User.findOneAndUpdate({ _id: user._id, balance: { $gte: amount } }, { $inc: { balance: -amount } }, { new: true });
    if (!deductedUser) return res.status(400).json({ message: "Insufficient balance" });

    let vtuResponse;
    try {
        vtuResponse = await buyEPIN(network, value, quantity);
    } catch(err) {
        await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
        return res.status(500).json({ message: "Service unavailable. Refunded." });
    }
    ...
    if (vtuResponse.status === "success") {
       await Transaction.create({
         ...
         amount, network,
         reference: vtuResponse.reference,
         description: `${network} EPIN (Qty: ${quantity})`,
         token: vtuResponse.token
       });
       res.json({ message: "Purchase successful", balance: deductedUser.balance, token: vtuResponse.token });
    } else {
...
app.post("/buy-education", auth, async (req, res) => {
  try {
    const { amount, phone, examType } = req.body;
    if (!amount || amount <= 0) return res.status(400).json({ message: "Invalid amount" });
    ...
    let vtuResponse;
    try {
        vtuResponse = await buyEducation(examType, phone);
    } catch(err) {
        await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
        return res.status(500).json({ message: "Service unavailable. Refunded." });
    }
    ...
    if (vtuResponse.status === "success") {
       await Transaction.create({
         userId: user._id, type: "debit", status: "success", amount, phone, network: examType,
         reference: vtuResponse.reference, description: `Education PIN - ${examType}`, token: vtuResponse.token
       });
       res.json({ message: "Purchase successful", balance: deductedUser.balance, token: vtuResponse.token });
    } else {
       await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
...
app.post("/buy-data", auth, async (req, res) => {
  try {
    const { amount, phone, network, dataPlan } = req.body;
    if (!amount || amount <= 0) return res.status(400).json({ message: "Invalid amount" });
    ...
    let vtuResponse;
    try { vtuResponse = await buyData(network, dataPlan, phone); } 
    catch(err) {
        await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
        return res.status(500).json({ message: "Service unavailable. Refunded." });
    }
    ...
    if (vtuResponse.status === "success") {
       await Transaction.create({
         userId: user._id, type: "debit", status: "success", amount, phone, network,
         reference: vtuResponse.reference, description: `${network} Data - ${dataPlan}`
       });
       res.json({ message: "Data purchased successfully", balance: deductedUser.balance });
    } else {
       await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
       res.status(400).json({ message: vtuResponse.message || "Failed. Refunded.", balance: deductedUser.balance + amount });
    }
...
app.post("/buy-cable", auth, async (req, res) => {
  try {
    const { amount, phone, cableId, packageId, smartcard } = req.body;
    if (!amount || amount <= 0) return res.status(400).json({ message: "Invalid amount" });
    ...
    let vtuResponse;
    try { vtuResponse = await buyCableTV(cableId, packageId, smartcard, phone); } 
    catch(err) {
        await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
        return res.status(500).json({ message: "Service unavailable. Refunded." });
    }
    ...
    if (vtuResponse.status === "success") {
       await Transaction.create({
         userId: user._id, type: "debit", status: "success", amount, phone, network: cableId,
         reference: vtuResponse.reference, description: `${cableId} Sub - ${smartcard}`
       });
       res.json({ message: "Subscription successful", balance: deductedUser.balance });
    } else {
       await User.findByIdAndUpdate(user._id, { $inc: { balance: amount } });
       res.status(400).json({ message: vtuResponse.message || "Failed. Refunded.", balance: deductedUser.balance + amount });
    }

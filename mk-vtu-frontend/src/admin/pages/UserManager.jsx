...
const UserManager = ({ token, type }) => {
  ...
  const titleMap = {
    ...
    'reseller-customers': 'Reseller Sub-Users',
    'developers': 'API Developers',
    'default': 'Global User Manager'
  };
  ...
  // Handle smart shortcuts from ResellerManager
  useEffect(() => {
    if (location.state?.openWalletFor) {
      setSelectedUser(location.state.openWalletFor);
      if (location.state?.walletAction) {
        setWalletAction(location.state.walletAction);
      }
      setShowWalletModal(true);
      // Clean up the state so it doesn't reopen on reload
  ...
  const fetchUsers = () => {
    setLoading(true);
    API.get(`/api/admin/users?search=${search}&type=${type || ''}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => {
        setUsers(res.data);
      })
      .catch(err => {
  ...
  return (
    <div className="manager-wrapper">
      <div className="manager-header">
        <h2>{titleMap[type] || titleMap['default']}</h2>
        ...
        <table className="admin-table">
          <thead>
            <tr>
              <th>User</th>
              <th>Email</th>
              {type === 'reseller-customers' && <th>Referrer (Reseller)</th>}
              <th>Type / API</th>
              <th>White Label</th>
              <th>Status</th>
              <th>Actions</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={type === 'reseller-customers' ? "7" : "6"} style={{ textAlign: 'center', padding: '40px' }}>Loading Users...</td></tr>
            ) : users.length === 0 ? (
              <tr><td colSpan={type === 'reseller-customers' ? "7" : "6"} style={{ textAlign: 'center', padding: '40px' }}>No users found.</td></tr>
            ) : users.map(user => (
              <tr key={user._id}>
                <td>
                ...
                <td>{user.email}</td>
                {type === 'reseller-customers' && (
                  <td>
                    {user.referredBy ? (
                      <div className="referrer-info">
                        <strong style={{ display: 'block', fontSize: '14.3px' }}>{user.referredBy.branding?.siteName || user.referredBy.name}</strong>
                        <small style={{ color: 'var(--text-light)', fontSize: '12.1px' }}>{user.referredBy.email}</small>
                      </div>
                    ) : <span className="none">Main Platform</span>}
                  </td>
                )}
                <td>
                  <div className="api-cell">
                    <span className={`badge ${user.apiLevel}`}>{user.apiLevel?.toUpperCase()}</span>
                  </div>
                </td>
                <td>
                  {user.subdomain || user.customDomain ? (
                    <div className="wl-status">
                       <span className={`badge ${user.whiteLabelStatus}`}>{user.whiteLabelStatus?.toUpperCase()}</span>
                       <small>{user.subdomain ? `${user.subdomain}.9jasub.com` : user.customDomain}</small>
                    </div>

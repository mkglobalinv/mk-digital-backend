...
async function verifyReset() {
    try {
        ...
        const report = {
            ...
            deletedCounts: {}, // Assuming deleted since we ran cleanup.js
            verification: {}
        };
        ...
        for (const col of collectionsToClear) {
            const count = await col.model.countDocuments({}).maxTimeMS(5000);
            report.collectionsCleaned.push(col.name);
            report.deletedCounts[col.name] = `Current count: ${count}`; // Should be 0
        }

        console.log("Checking Users...");
        const userCount = await User.countDocuments({}).maxTimeMS(5000);
        report.collectionsCleaned.push('Users');
        report.deletedCounts['Users'] = `Current count: ${userCount}`; // Should be small

        console.log("Verifying state...");
        
        // Admin unuktar1@gmail.com exists
        const adminUser = await User.findOne({ email: 'unuktar1@gmail.com' }).maxTimeMS(5000).lean();
        report.verification.adminExists = !!adminUser;
        
        // Super Admins exist
        const superAdminsCount = await User.countDocuments({ role: 'superadmin' }).maxTimeMS(5000);
        report.verification.superAdminsExist = superAdminsCount > 0;

        // System Settings intact and pricing check
        const sysSettings = await SystemSetting.findOne({}).maxTimeMS(5000).lean();
        ...
        // Data Plans intact
        const dataPlansCount = await DataPlan.countDocuments({}).maxTimeMS(5000);

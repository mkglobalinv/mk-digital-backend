...
Each row is its own `DataPlan` document (`provider: 'ogdams'`), so cost (`api_price`) and every pricing tier -- Retail (`selling_price`, what a direct customer pays), Basic (`reseller_price`), VIP (`vip_price`), Premium (`premium_price`) -- plus the auto-computed profit and active status are stored per-document exactly as they already are for every other provider -- editing an Ogdams row can never change a Peyflex `DataPlan` document, and vice versa; no new schema or pricing computation was introduced. These are the same 4 tiers "Legacy Data Pricing" already manages for Peyflex, and the same tiers `calculateVtuPrice`/`calculateBulkDataPrices` (`services/pricing/vtuPricing.js`) already read from when pricing a reseller's purchase or a white-label reseller website's storefront (falling back to these per-plan tier prices unless that specific reseller has a `PriceOverride`/`assignedPrices` entry). Edits go through the existing `PUT /admin/data-plans/:id` (now additionally accepting `api_price`, with basic non-negative validation matching `selling_price`'s existing validation).
...
### V3 Pricing Rule support (provider-scoped)

The platform's actual pricing-management workflow is the "V3 Pricing Engine" (Admin → V3 Pricing Rules, `models/PricingRule.js`, `routes/adminRoutes.js`'s `applyPricingRuleToPlans`): a `PricingRule` per `(network, category)` stores `retailPercentage`/`basicPercentage`/`vipPercentage`, and saving it bulk-recomputes `selling_price`/`reseller_price`/`vip_price`/`premium_price` (and their `_selling_price` counterparts) on every matching `DataPlan` from that plan's own `api_price`. The per-plan raw price fields the Ogdams SME Pricing page also exposes are what this engine writes to -- editing them directly is a one-off override that lasts until the rule is next applied.

`PricingRule` gained an optional `provider` field (default `undefined`) so this engine can manage Ogdams' MTN SME pricing independently:
- A rule with `provider` set (e.g. `'ogdams'`) only ever recomputes that provider's plans for the network+category.
- The original, provider-less "default" rule still recomputes every provider's plans for that network+category **except** any provider that has its own active provider-scoped rule -- so once an Ogdams-specific rule exists, saving the shared/default MTN Gifting rule (used for Peyflex) can never overwrite Ogdams' plans, and vice versa. This decision is pure and unit-tested as `pricingRuleProviderFilter()`; the markup math itself is `computeV3Prices()` (`tests/pricingRuleV3.test.js`).
- The unique index moved from `{network, category}` to `{network, category, provider}` -- backward compatible, since every existing rule already satisfies uniqueness on `{network, category}` alone with `provider` absent.
- `GET /admin/pricing-rules` accepts optional `network`/`category`/`provider` filters; `POST /admin/pricing-rules` accepts an optional `provider` in its body and targets exactly the matching rule (default vs. that provider) rather than whichever rule happens to match `{network, category}` first.
- The bulk-generator and clone-network routes (`POST /pricing-rules/bulk`, `POST /pricing-rules/clone`) now explicitly scope to default (no-provider) rules only, so they can never touch a provider-scoped rule.

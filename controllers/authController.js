    ...
    await Session.create({ userId: user._id, token, deviceInfo: device });
        ...
        setImmediate(async () => {
            try {
                ...
                console.error("Failed to send login alerts on verification:", alertErr);
            }
        });
...
export const resetTransactionPin = async (req, res) => {
    try {
        ...
        let decoded;
        try {
            decoded = jwt.verify(resetToken, process.env.JWT_SECRET || "mk_sub_data_secret_2024_premium");
        } catch (tokenErr) {
